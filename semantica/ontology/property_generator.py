...
class PropertyGenerator:
        ...
        **options
        ...
        for rel_type, rels in rel_types.items():
            if len(rels) >= options.get("min_occurrences", 2):
                ...
                for rel in rels:
                    source_type = rel.get("source_type") or self._infer_class_from_entity(rel.get("source_id"), classes)
                    target_type = rel.get("target_type") or self._infer_class_from_entity(rel.get("target_id"), classes)
                ...
                prop_name = self.naming_conventions.normalize_property_name(rel_type, "object")
                
                property_def = {
                    "name": prop_name,
                    "type": "object",
                    "uri": options.get("namespace_manager", None).generate_property_iri(prop_name) if options.get("namespace_manager") else None,
                    "label": prop_name,
    ...
    def _infer_data_properties(
        ...
        **options
        ...
        for entity_type, type_entities in entity_types.items():
            ...
            class_def = next((cls for cls in classes if cls["name"] == entity_type), None)
            if not class_def:
                continue
            ...
            for prop_name, prop_type in data_props.items():
                # Normalize property name
                normalized_name = self.naming_conventions.normalize_property_name(prop_name, "data")
                
                property_def = {
                    "name": normalized_name,
                    "type": "data",
                    "uri": options.get("namespace_manager", None).generate_property_iri(normalized_name) if options.get("namespace_manager") else None,
                    "label": normalized_name,
                    "comment": f"Data property for {prop_name}",
                    "domain": [entity_type],
                    "range": prop_type,
                    "metadata": {
                        "inferred_from": prop_name
                    }
                }
    ...
    def _extract_data_properties(self, entities: List[Dict[str, Any]]) -> Dict[str, str]:
        ...
        for entity in entities:
            for key, value in entity.items():
                ...
                if key not in properties:
                    ...
                    properties[key] = self._get_more_general_type(properties[key], prop_type)
    ...
    def _infer_class_from_entity(self, entity_id: str, classes: List[Dict[str, Any]]) -> Optional[str]:
    ...
    def validate_properties(
        ...
        **criteria
        ...
        for prop in properties:
            ...
            is_valid, suggestion = self.naming_conventions.validate_property_name(prop["name"], prop_type)
            if not is_valid:
                warnings.append(f"Property '{prop['name']}' doesn't follow conventions. Suggested: {suggestion}")
