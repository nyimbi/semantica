            ...
            if file_path.suffix in ['.yaml', '.yml']:
                ...
                try:
                    ...
                    try:
                        ...
                        self.logger.warning("TOML parser not available. Install tomli or use Python 3.11+")
    ...
    def _load_from_env(self) -> None:
        ...
        for env_var, config_key in env_mappings.items():
            ...
            if value is not None:
                ...
                if config_key in ['node_size', 'edge_width', 'point_size', 'umap_n_neighbors', 
                                  'pca_n_components', 'force_layout_iterations']:
                    try:
                        self._config[config_key] = int(value)
                    except ValueError:
                        self.logger.warning(f"Invalid integer value for {env_var}: {value}")
                elif config_key in ['tsne_perplexity', 'force_layout_k', 'hierarchical_vertical_spacing', 
                                    'circular_radius']:
                    try:
